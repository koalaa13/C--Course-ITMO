...
void shift_array_left(my_vector &arr, u_int const &shift) {
    u_int big = shift / BIT, small = shift % BIT;
    my_vector res(big + arr.size());
...
void shift_array_right(my_vector &arr, u_int const &shift) {
    u_int big = shift / BIT, small = shift % BIT;
    if (big >= arr.size()) {
        arr.resize(1);
        return;
    }
    my_vector res(arr.size() - big);
...
my_vector make_two_complement(my_vector const &arr, bool const &s) {
    my_vector res = arr;
    res.push_back(0);
    if (s) {
        for (size_t i = 0; i < res.size(); ++i) {
...
big_integer::big_integer(my_vector const &arr) {
    my_vector copy = arr;
    if (copy.back() == 0) {
        copy.pop_back();
        sign = false;
...
big_integer big_integer::mul_long_short(u_int const &b) const {
    ...
    if (prev) {
        res.data[res.data.size() - 1] = prev % RADIX;
    } else {
        res.data.pop_back();
    }
    res.delete_zeroes();
...
void big_integer::add_eq_long_short(u_int const &b) {
    ...
    for (size_t i = 0; i < data.size(); ++i) {
        ...
        data[i] = static_cast<u_int>(cur % RADIX);
        prev = cur / RADIX;
        if (prev == 0) {
            break;
        }
...
big_integer big_integer::unsigned_sub_long(big_integer const &a, big_integer const &b) {
    big_integer res = a;
    res.sign = false;
    bool borrowed = false;
...
const big_integer operator-(big_integer const &a, big_integer const &b) {
    ...
    if (a < b) {
        return -big_integer::unsigned_sub_long(b, a);
    } else {
        return big_integer::unsigned_sub_long(a, b);
    }
}
...
const big_integer &operator++(big_integer &a) {
    if (a.sign) {
        ...
        for (size_t i = 0; i < a.data.size(); ++i) {
            u_ll cur = static_cast<u_ll>(a.data[i]) + static_cast<u_ll>(was);
            if (cur == RADIX) {
                a.data[i] = 0;
            } else {
                a.data[i]++;
...
const big_integer operator&(big_integer const &a, big_integer const &b) {
    my_vector first = make_two_complement(a.data, a.sign), second = make_two_complement(b.data, b.sign);
...
const big_integer operator~(big_integer const &a) {
    my_vector res = make_two_complement(a.data, a.sign);
    for (size_t i = 0; i < res.size(); ++i) {
        res[i] = ~res[i];
    }
    return big_integer(res);
